...
feature "secret features" do 
  before do 
    ...
    @secret2 = create(:secret, user: @user2, context: "Lets make this context unique")
    log_in
  end
  ...
  feature "Users personal secret page" do
    ...
    scenario "shouldn't see other user's secrets" do 
      expect(page).to_not have_text(@secret2.context)
    end    
    scenario "create a new secret" do
      fill_in 'context', with: "My new secret"
    ...
    scenario "destroy secret from profile page, redirects to user profile page" do
      ...
      expect(current_path).to eq("/users/#{@user.id}")
      expect(page).not_to have_text(secret.context)
    end    
  end
  feature "Secret Dashboard" do 
    ...
    scenario "displays everyone's secrets" do 
      expect(page).to have_text(@secret.context)
      expect(page).to have_text(@secret2.context)      
    end
